...
exports.AssetDiscoveryService = exports.MAX_SNAPSHOT_WIDTHS = void 0;
...
class AssetDiscoveryService extends percy_client_service_1.default {
    constructor(buildId, configuration) {
        ...
        this.configuration = configuration ||
            configuration_1.DEFAULT_CONFIGURATION.agent['asset-discovery'];
        this.responseService = new response_service_1.default(buildId, this.configuration['allowed-hostnames'], this.configuration['cache-responses']);
    }
    async setup() {
    ...
    async discoverResources(rootResourceUrl, domSnapshot, options, logger) {
        ...
        logger.debug(logger_1.addLogDate(`discovering assets for URL: ${rootResourceUrl}`));
    ...
    shouldProcessRequest(resourceUrl, rootResourceUrl) {
        ...
        if (this.configuration['allowed-hostnames'].some((hostname) => domain_match_1.default(hostname, resourceUrl))) {
            return true;
        }
    ...
    async resourcesForWidth(pool, width, domSnapshot, rootResourceUrl, enableJavaScript, requestHeaders = {}, logger) {
        logger.debug(logger_1.addLogDate(`discovering assets for width: ${width}`));
        ...
        page.on('request', async (request) => {
            ...
            try {
                ...
                if (!this.shouldProcessRequest(requestUrl, rootResourceUrl)) {
                    logger.debug(logger_1.addLogDate(`Aborting ${requestUrl} -- will never be saved`));
                    await request.abort();
                    return;
                }
                if (this.configuration['cache-responses'] === true && response_cache_1.getResponseCache(requestUrl)) {
                    logger.debug(logger_1.addLogDate(`Asset cache hit for ${requestUrl}`));
                    await request.respond(response_cache_1.getResponseCache(requestUrl));
                    return;
                }
                logger.debug(logger_1.addLogDate(`Starting processing for: ${requestUrl}`));
        ...
        page.on('requestfinished', async (request) => {
            ...
            if (response) {
                ...
                logger.debug(logger_1.addLogDate(`No response for ${request.url()}. Skipping.`));
            }
        });
        // Debug log failed requests.
        page.on('requestfailed', async (request) => {
            logger.debug(logger_1.addLogDate(`Failed to load ${request.url()} : ${request.failure().errorText}}`));
        });
        let maybeResources = [];
        try {
            ...
            logger.error(logger_1.addLogDate(`${error.name} ${error.message}`));
            logger.debug(logger_1.addLogDate(error));
        }
        try {
